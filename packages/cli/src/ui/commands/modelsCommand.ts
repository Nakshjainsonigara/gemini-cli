...
function formatModelsList(registry: ModelRegistry): string {
  ...
  const sections = providers.map(provider => {
    ...
    const apiKeyStatus = hasApiKey ? '✓ API key configured' : '⚠ API key required';
...
export const modelsCommand: SlashCommand = {
  name: 'models',
  altNames: ['model'],
  description: 'Manage AI models and providers. Usage: /models [list|set|key]',
  ...
  action: async (context: CommandContext, args: string): Promise<SlashCommandActionReturn> => {
    ...
    switch (subCommand) {
      ...
      case 'set':
        if (parts.length < 2) {
          return {
            type: 'message',
            messageType: 'error',
            content: 'Usage: /models set <provider> <model>\nExample: /models set openai gpt-4o',
          } as MessageActionReturn;
        }
        ...
        context.services.settings.setValue(SettingScope.User, 'modelRegistry', registry.toJSON());
      ...
      case 'apikey':
        if (parts.length < 3) {
          return {
            type: 'message',
            messageType: 'error',
            content: 'Usage: /models key <provider> <api-key>\nExample: /models key openai sk-...',
          } as MessageActionReturn;
        }
      ...
      case 'current':
        ...
        const currentProviderConfig = registry.getProvider(registry.getCurrentProvider());
