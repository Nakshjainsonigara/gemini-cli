...
describe('ModelRegistry', () => {
  ...
  describe('model management', () => {
    ...
    it('should fail to set invalid model', () => {
      const registry = new ModelRegistry();
      const success = registry.setCurrentModel(
        ModelProvider.OPENAI,
        'invalid-model',
      );
  ...
  describe('API key management', () => {
    ...
    it('should fail to set API key for invalid provider', () => {
      const registry = new ModelRegistry();
      const success = registry.setProviderApiKey(
        'invalid' as ModelProvider,
        'key',
      );
  ...
  describe('serialization', () => {
    it('should serialize to JSON and restore from JSON', () => {
      ...
      expect(registry2.getProviderApiKey(ModelProvider.ANTHROPIC)).toBe(
        'test-key',
      );
    });
  });
