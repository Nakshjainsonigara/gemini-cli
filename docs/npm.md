...
This monorepo contains two main packages: `@google/ai-cli` and `@google/ai-cli-core`.

## `@google/ai-cli`

This is the main package for the AI CLI. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@google/ai-cli-core`. This means that whether a user installs the package with `npm install -g @google/ai-cli` or runs it directly with `npx @google/ai-cli`, they are using this single, self-contained executable.
...
Releases are managed through the [release.yml](https://github.com/Nakshjainsonigara/ai-cli/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
Every night at midnight UTC, the [Release workflow](https://github.com/Nakshjainsonigara/ai-cli/actions/workflows/release.yml) runs automatically on a schedule. It performs the following steps:
...
```bash
npm install -g @google/ai-cli@nightly
```
...
After the workflow has successfully completed, you can monitor its progress in the [GitHub Actions tab](https://github.com/Nakshjainsonigara/ai-cli/actions/workflows/release.yml). Once complete, you should:

1.  Go to the [pull requests page](https://github.com/Nakshjainsonigara/ai-cli/pulls) of the repository.
...
- `npx -y @google/ai-cli@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @google/ai-cli@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @google/ai-cli && npm uninstall -g @google/ai-cli && npm cache clean --force &&  npm install @google/ai-cli@<version>`
...
1.  Go to the [Actions tab](https://github.com/Nakshjainsonigara/ai-cli/actions/workflows/release.yml) of the repository.
...
1.  The `package.json` is Transformed:
    ...
    - Why: The final package.json must be different from the one used in development. Key changes include:
      ...
      - Removing workspace-specific "dependencies": { "@ai-cli/core": "workspace:\*" } and ensuring the core code is
...
2.  The JavaScript Bundle is Created:
    ...
    - File movement: packages/cli/dist/index.js + packages/core/dist/index.js -> (bundled by esbuild) -> `bundle`/ai.js (or a
      similar name).
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @google/ai-cli`.
